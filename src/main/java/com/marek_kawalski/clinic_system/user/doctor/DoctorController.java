...
public class DoctorController {
    ...
    public ResponseEntity<List<LocalDateTime>> getAvailableAppointments(@PathVariable final String id, @PathVariable final String examinationId,
                                                                        @PathVariable final LocalDateTime date) {
        try {
            ...
            return availableAppointments.isEmpty() ? ResponseEntity.status(HttpStatus.NO_CONTENT).body(null)
                    : ResponseEntity.status(HttpStatus.OK).body(availableAppointments);
